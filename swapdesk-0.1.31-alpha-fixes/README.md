...
**v0.1.31-alpha** | <https://github.com/swapdesk/swapdesk>
...
cd swapdesk/SwapDesk
```

The application lives in `SwapDesk/`, and every path in this README is
relative to that folder. The exceptions sit at the repo root, one level up:
this README, `LICENSE` and `SECURITY.md`.
...
report Unknown rather than guess. Of the two, only Maya routes DASH. Through
either, the coin you send must be BTC, LTC, DOGE, BCH or DASH: ETH deposits
have to go through the protocol's router contract, and XMR and SOL deposits
need memo handling SwapDesk cannot verify, so those are offered as coins to
receive only.
...
the project's affiliate account, which is what funds the work. SwapDesk adds
no fee of its own on top of the provider's quote: any commission the provider
pays on the swap is already inside the quote you're shown, and SwapDesk still
never receives, holds, or forwards your coins.
...
```
README.md                this file. It sits at the repo root, one level
                           above the paths listed below, and the app folder
                           carries no second copy to drift out of sync.
                           build.py picks it up when packaging a binary
...
SECURITY.md           private vulnerability disclosure process (repo root)
...
Provider status mapping is checked against each provider's documented
vocabulary rather than against live swaps. Field names that their public
documentation does not state are marked as such in the code comments and were
not guessed at.
...
See [SECURITY.md](SECURITY.md) for the private vulnerability disclosure
...
Beyond what `preflight.py` verifies (address format, pair support, live
quotes), these are the realistic ways funds get lost
...
| Sent after the deposit window expired | `preflight.py` prints `expires_at`; once a deadline the app can read has passed, the deposit window withdraws its QR code and copy buttons | Providers that report no deadline, or one in an unrecognised format, get no lock; get a fresh quote if in doubt |
...
| Provider dishonesty / exit scam | Diversify across providers; start with small amounts; read [SECURITY.md](SECURITY.md) | Nothing client-side can verify a provider's actual on-chain behavior in advance |
