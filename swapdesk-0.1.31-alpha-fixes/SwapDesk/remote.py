...
from providers.constants import _epoch_utc
...
class RemoteSwapDesk(SwapProvider):
    ...
    def create_swap(self, from_coin, to_coin, amount, settle_address,
        ...
        if not isinstance(data, dict):
            raise ProviderError(
                f"{self.name}: unexpected non-object response from /swap/create. "
                f"Do not send funds. Response type: {type(data).__name__}")
        ...
        if echoed is not None and not isinstance(echoed, str):
            raise ProviderError(
                f"{self.name}: SAFETY ABORT: the server's destination echo is "
                f"not an address ({type(echoed).__name__}). Do not send "
                f"anything to the deposit address it returned.")
        ...
        return Swap(
            provider=self.name,
            order_id=self._checked_order_id(swap_id),
            from_coin=from_coin,
            to_coin=to_coin,
            ...
            expires_at=(expiration if isinstance(expiration, str)
                        else _epoch_utc(expiration)),
            raw=data,
