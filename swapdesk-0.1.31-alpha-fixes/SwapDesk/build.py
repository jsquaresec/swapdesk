...
# Shipped beside the binary. README, LICENSE and SECURITY live at the repo
# root so GitHub picks them up and there is one copy of each; VERSION is the
# app's own. package() looks in the app folder first, then one level up.
PACKAGE_DOCS = ["README.md", "LICENSE", "VERSION", "SECURITY.md"]
...
def write_baked(keys: dict) -> None:
    ...
    with os.fdopen(fd, "w", encoding="utf-8") as fh:
        fh.write(body)
    if os.name == "nt":
        # The 0o600 above is POSIX-only; NTFS access is governed by ACLs, not
        # those mode bits, so on Windows the file was otherwise left at
        # whatever it inherited from ROOT -- typically readable by every
        # local account -- for the duration of the build. Same icacls
        # approach config.py's _lock_down() uses for data/config.json:
        # strip inheritance, grant only the current user. Best-effort and
        # non-fatal like _lock_down(), since a failure here still leaves the
        # POSIX-mode file no more exposed than it already was without this.
        user = os.environ.get("USERNAME", "")
        if user:
            root = os.environ.get("SystemRoot") or os.environ.get("windir")
            icacls = str(Path(root) / "System32" / "icacls.exe") if root else "icacls"
            r = subprocess.run(
                [icacls, str(BAKED), "/inheritance:r", "/grant:r", f"{user}:F"],
                shell=False, capture_output=True, check=False)
            if r.returncode != 0:
                print(f"WARNING: could not restrict {BAKED.name} to the "
                      f"current user (icacls: "
                      f"{r.stderr.decode('utf-8', errors='ignore').strip()}). "
                      f"It is readable by other local accounts until the "
                      f"build deletes it.", file=sys.stderr)
