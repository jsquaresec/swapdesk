...
#
# Deliberately states no number. If a future bundled key carries a specific
# commission rate worth disclosing, that rate must come from
# SwapProvider.disclosed_commission_bps(), which reads it off the quote that
# was actually returned -- never a constant added here or in a provider
# file. A rate set on the provider's own side can change without a code
# change on ours, and a hardcoded figure would then keep quoting users a
# number the provider no longer honours, with nothing to catch the drift.
DISCLOSURE = (
    ...
    "affiliate account, which is how it's funded. SwapDesk adds no fee of its "
    "own: any commission the provider pays is already inside the quote you're "
    "shown, and the app still never touches your coins. Enter your own keys "
    "in Settings to route around it entirely."
)
