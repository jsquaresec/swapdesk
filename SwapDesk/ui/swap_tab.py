...
class SwapTabMixin:
    ...
    def _show_quotes(self, results: dict, apply_auto_select: bool = True,
        ...
        if bundled:
            self._bundled_row(sorted(bundled))
        if skipped:
    ...
    def _bundled_row(self, providers: list):
        ...
        names = ", ".join(p.title() for p in providers)
        ctk.CTkLabel(
            row,
            text=f"\u2139  {names}: using this build's bundled key. "
                 + bundled_keys.DISCLOSURE,
